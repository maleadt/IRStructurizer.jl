...
"""
    rename_ssa_def!(block::Block, old_idx::Int, new_idx::Int) -> Bool

Rename an SSA definition key from `old_idx` to `new_idx` within a block hierarchy.
Returns `true` if the definition was found and renamed.
"""
function rename_ssa_def!(block::Block, old_idx::Int, new_idx::Int)
    for b in eachblock(block)
        pos = findfirst(==(old_idx), b.body.ssa_idxes)
        if pos !== nothing
            b.body.ssa_idxes[pos] = new_idx
            return true
        end
    end
    return false
...
Rename SSA defs in `else_blk` that collide with defs in `then_blk`.
A convergence block (e.g., the true-target of `a || b`) may have been
collected by both branches of a REGION_PROPER.
"""
function deduplicate_branch_defs!(then_blk::Block, else_blk::Block,
                                   ctx::StructurizationContext)
    then_defs = Set{Int}()
    for b in eachblock(then_blk)
        for (idx, _) in b.body
            push!(then_defs, idx)
        end
    end
    # Collect conflicts first to avoid mutating during iteration.
    conflicts = Int[]
    for b in eachblock(else_blk)
        for (idx, _) in b.body
            idx ∈ then_defs && push!(conflicts, idx)
        end
    end
    for idx in conflicts
        new_idx = ctx.next_ssa_idx
        ctx.next_ssa_idx += 1
        rename_ssa_def!(else_blk, idx, new_idx)
        replace_uses!(else_blk, SSAValue(idx), SSAValue(new_idx))
    end
end

"""
    rename_extra_exit_defs!(extra_exits, ctx, carried_values, blocks...)

Rename extra exit value definitions to fresh SSA indices inside the loop body.
The original index will be reused by the outer getfield extraction (handle_loop!),
so the inner definition must not collide.
"""
function rename_extra_exit_defs!(extra_exits, ctx::StructurizationContext,
                                  carried_values::Vector{IRValue}, blocks::Block...)
    for ex in extra_exits
        old_idx = ex.getfield_idx
        new_idx = ctx.next_ssa_idx
        if any(b -> rename_ssa_def!(b, old_idx, new_idx), blocks)
            ctx.next_ssa_idx += 1
            for b in blocks
                replace_uses!(b, SSAValue(old_idx), SSAValue(new_idx))
            end
            for (i, v) in enumerate(carried_values)
                v isa SSAValue && v.id == old_idx && (carried_values[i] = SSAValue(new_idx))
            end
        end
    end
end
